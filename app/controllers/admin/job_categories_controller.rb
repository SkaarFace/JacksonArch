class Admin::JobCategoriesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @job_category.save
        format.html { redirect_to [:admin, @job_category], notice: 'Job category was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @job_category.update(job_category_params)
        format.html { redirect_to [:admin, @job_category], notice: 'Job category was successfully updated.' }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to admin_job_categories_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def job_category_params
      params.require(:job_category).permit(:name, :job_type => [:id, :name])
    end
end
