class Admin::JobTypesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @job_type.save
        format.html { redirect_to [:admin, @job_type], notice: 'Job type was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @job_type.update(job_type_params)
        format.html { redirect_to [:admin, @job_type], notice: 'Job type was successfully updated.' }
  ...
  private
    ...
    def job_type_params
      params.require(:job_type).permit(:name, :job_category_id)
    end
end
